...
| Sliding window | `apps/web/lib/timing.ts` | `RollingAverage.push` | O(1) | (vitest not installed; RollingAverage is visibly used in reader UI) | Rolling-WPM display in reader (~{N} WPM, updates every 2s) | Row confirmed — RollingAverage is wired into page.tsx and visible during reading phase |
